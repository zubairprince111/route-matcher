...
export function ListingsBottomSheet({ tickets, onReportTicket }: ListingsBottomSheetProps) {
    ...
    return (
        ...
        >
            <DrawerContent
                className="fixed bottom-0 left-0 right-0 z-[1000] bg-[#F8F9FA] border-t-0 rounded-t-3xl shadow-2xl flex flex-col focus-visible:outline-none focus:outline-none mx-auto w-full md:max-w-2xl px-0 overflow-hidden"
                style={{ height: '85vh' }}
            >
                {/* Drag Handle + Header */}
                <div
                    className="bg-white rounded-t-3xl cursor-grab active:cursor-grabbing px-4 pt-3 pb-4 shrink-0 border-b border-slate-100"
                    onClick={() => setSnap(isExpanded ? 0.35 : 0.85)}
                >
                    {/* Handle bar */}
